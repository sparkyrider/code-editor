...
function EntryView({ entry, hasBg }: { entry: TerminalEntry; hasBg: boolean }) {
  ...
  if (entry.type === 'command') {
    return (
      <div
        className={`text-[13.5px] leading-[1.7] ${backdrop}`}
  ...
  if (entry.type === 'system') {
    return (
      <div
        className={`text-[12.5px] leading-[1.7] text-[var(--text-disabled)] ${backdrop}`}
        style={{ fontFamily: MONO, ...bgStyle }}
      >
        {entry.text}
      </div>
  ...
  if (entry.type === 'error') {
    return (
      ...
      >
        <span className="font-medium text-[var(--color-deletions)]">{entry.text}</span>
      </div>
    )
  }
...
export function GatewayTerminal() {
  ...
  // External focus hook (keyboard shortcut / command palette)
  useEffect(() => {
